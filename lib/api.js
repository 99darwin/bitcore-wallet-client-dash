...
var BN = require('divicore-lib/lib/crypto/bn');
...
var BASE_URL = 'http://174.138.14.81:3232/bws/api';

var VALID_COINS = ['divi', 'btc', 'bch'];
...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.coin || defaultCoin, opts.network || 'livenet');
};
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = DefaultCore.HDPublicKey.fromString('ToEA6kdSDwa2hjD4R96h7jpcj6vCs53BMzSDTSoWAfiMU5jmsXEGxNpmSooGwhaUVsbNTcudi4Gf1AcACxJhfudFhgVT6NjbGN1dShiq7DZjnJg');
    return testMessageSigning(xpriv, xpub, defaultCoin);
  };
  ...
  var liveOk = (c.canSign() && !c.isPrivKeyEncrypted()) ? testLiveKeys(this.credentials.coin || defaultCoin) : true;
 ...
 *
 * @param {Object} opts
 * @param {String} opts.coin - default
 * @param {String} opts.network - default 'livenet'
 * @param {String} opts.passphrase
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.coin || defaultCoin, opts.network || 'livenet', opts.passphrase, opts.language || 'en', opts.account || 0);
};
...
API.prototype.seedFromExtendedPrivateKey = function(xPrivKey, opts) {
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPrivateKey(opts.coin || defaultCoin, xPrivKey, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.coin || defaultCoin, opts.network || 'livenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.seedFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPublicKey(opts.coin || defaultCoin, xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44);
};
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  function derive(nonCompliantDerivation) {
    return Credentials.fromMnemonic(opts.coin || defaultCoin, opts.network || 'livenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, {
...
API.prototype.importFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, opts, cb) {
  ...
  log.debug('Importing from Extended Private Key');
  try {
    this.credentials = Credentials.fromExtendedPublicKey(opts.coin || defaultCoin, xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
  } catch (e) {
    log.info('xPriv error:', e);
    return cb(new Errors.INVALID_BACKUP);
  };
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var coin = opts.coin || (self.credentials || {}).coin || defaultCoin;
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, coin, cb) {
  ...
  var privateKey = new Bitcore_[coin].PrivateKey(privateKey);
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  var coin = opts.coin || defaultCoin;
  var privateKey = Bitcore_[coin].PrivateKey(privateKey);
  var address = privateKey.publicKey.toAddress();
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var tx;
      try {
        var toAddress = Bitcore_[coin].Address.fromString(destinationAddress);
  ...
  if (self.credentials) {
    ...
    if (useSession && self.session) {
      ...
      if (key) {
        ...
        reqSignature = API._signRequest(method, url, args, key, self.credentials.coin);
      }
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var coin = secretSplit[3] || defaultCoin;
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Bitcore_[txp.coin || defaultCoin].HDPrivateKey(derivedXPrivKey);
...
API.prototype._signTxp = function(txp, password) {
  var derived = this.credentials.getDerivedXPrivKey(password, txp.coin || this.credentials.coin);
...
API.prototype._addSignaturesToBitcoreTx = function(txp, t, signatures, xpub) {
  ...
  var i = 0,
    x = new Bitcore_[txp.coin].HDPublicKey(xpub);
  ...
  _.each(signatures, function(signatureHex) {
    ...
    try {
      ...
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: Bitcore_[txp.coin].crypto.Signature.SIGHASH_ALL | Bitcore_[txp.coin].crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  self._doGetRequest('/v2/feelevels/?coin=' + (coin || defaultCoin) + '&network=' + (network || 'livenet'), function(err, result) {
    if (err) return cb(err);
 ...
 * @param {String} copayerName
 * @param {Object} opts
 * @param {string} opts.coin The expected coin for this wallet. 
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var coin = c.coin;
    var walletPrivKey = Bitcore_[coin].PrivateKey.fromString(c.walletPrivKey);
...
API.prototype.fetchPayPro = function(opts, cb) {
  ...
  PayPro.get({
    ...
    coin: this.credentials.coin || defaultCoin
  }, function(err, paypro) {
    if (err)
      return cb(err);
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var coin = opts.coin || this.credentials.coin || defaultCoin;

  var reqPrivKey = new Bitcore_[coin].PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new Bitcore_[coin].HDPrivateKey(this.credentials.xPrivKey)
 ...
 */
  ...
  * @returns {Callback} cb - Returns error or an object with status information
  */
API.prototype.allocateFunds = function(opts, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  opts = opts || {};
  this._doPostRequest('/v1/allocatefunds/', opts, cb);
};
