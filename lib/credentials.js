...
Credentials.prototype._expand = function() {
  ...
  var coin = this.coin;
  var network = Credentials._getNetworkFromExtendedKey(this.xPrivKey || this.xPubKey, coin);
  if (this.network) {
  ...
  if (this.entropySource) {
    ...
    var privKey = new Bitcore_[coin].PrivateKey(seed.toString('hex'), network);
...
Credentials.prototype.getBaseAddressDerivationPath = function() {
  ...
  if (this.coin != 'divi')
    ...
    var coin = (this.network == 'livenet' ? "301" : "1");
...
Credentials.prototype.getDerivedXPrivKey = function(password, coin) {
  ...
  var xPrivKey = new Bitcore_[this.coin || defaultCoin].HDPrivateKey(this.getKeys(password).xPrivKey, this.network);
  var deriveFn = !!this.compliantDerivation ? _.bind(xPrivKey.deriveChild, xPrivKey) : _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey);
