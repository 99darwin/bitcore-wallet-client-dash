...
PayPro.get = function(opts, cb) {
  ...
  http(opts, function(err, dataBuffer) {
    ...
    var addr = new Bitcore_[coin].Address.fromScript(new Bitcore_[coin].Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount, coin) {
  ...
  var addr = new Bitcore_[coin].Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = Bitcore_[coin].Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = Bitcore_[coin].Script.buildScriptHashOut(addr);
  } else {
