...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network, coin) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new Bitcore_[coin].HDPublicKey(item.xPubKey);
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      bitcoreAddress = Bitcore_[coin].Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      bitcoreAddress = Bitcore_[coin].Address.fromPublicKey(publicKeys[0], network);
      break;
  }
...
Utils.xPubToCopayerId = function(coin, xpub) {
  var str = coin == defaultCoin ? xpub : coin + xpub;
...
Utils.buildTx = function(txp) {
  ...
  if (txp.toAddress && txp.amount && !txp.outputs) {
    ...
    _.each(txp.outputs, function(o) {
      ...
      if (o.script) {
        t.addOutput(new Bitcore_[coin].Transaction.Output({
          script: o.script,
          satoshis: o.amount
        }));
      } else {
